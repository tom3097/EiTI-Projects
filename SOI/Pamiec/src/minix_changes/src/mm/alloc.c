...
PRIVATE int strategy = 0;	/* strategia odnosnie algorytmu */
...
{
  struct hole *iterator;
  phys_clicks buffer[NR_HOLES * 2 + 1];
  phys_clicks pairNumber;
  phys_clicks temp_val;
  int index;
  int i;

  iterator = hole_head;

  temp_val = mm_in.m1_i1 / sizeof(phys_clicks);
  if(temp_val%2 == 0)
    pairNumber = (temp_val - 2)/2;
  else
    pairNumber = (temp_val - 1)/2;

  index = 0;
  i = 0;

  while(i < pairNumber && iterator != NIL_HOLE && iterator->h_base < swap_base)
  {
    buffer[index++] = iterator->h_len;
    buffer[index++] = iterator->h_base;

    iterator = iterator->h_next;
    ++i;
  }
  buffer[index] = 0;

  sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D, (phys_clicks)
	  mm_in.m1_p1, (phys_clicks) mm_in.m1_i1);
 ...
 */
  ...
  struct hole *max_hole;
  struct hole *prev_max_hole;
 ...
 /* do
    {
      ...
      while (hp != NIL_HOLE && hp->h_base < swap_base)
      {
        if ((hp->h_next == NIL_HOLE)&&(hp->h_len >= clicks))
        {
          old_base = hp->h_base;
	  hp->h_base +=clicks;
	  hp->h_len -= clicks;
