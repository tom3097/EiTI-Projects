...
FORWARD _PROTOTYPE( int do_getsizef, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getsizes, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getheadnr, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getheadpid, (message *m_ptr) );

PRIVATE int do_getpri(m_ptr)
message *m_ptr;
{
  struct proc* aux_proc;
  aux_proc = rdy_head[USER_Q];
  while(aux_proc != NIL_PROC && aux_proc->p_pid != m_ptr->m1_i1)
  {
    aux_proc = aux_proc->p_nextready;
  }

  if(aux_proc == NIL_PROC)
    return (ESRCH);
  else
    return aux_proc->NR_KOLEJKI;
}
...
message *m_ptr;
{
  ...
  if(aux_proc == rdy_head[USER_Q])
  {
    rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
    if(rdy_tail[USER_Q] == aux_proc)
    {
      rdy_tail[USER_Q] = NIL_PROC;
    }
    aux_proc->p_nextready = NIL_PROC;
  }
  else
  {
    bef_aux->p_nextready = aux_proc->p_nextready;
    if(rdy_tail[USER_Q] == aux_proc)
    {
      rdy_tail[USER_Q] = bef_aux;
    }
    aux_proc->p_nextready = NIL_PROC;
  }

  aux_proc->NR_KOLEJKI = m_ptr->m1_i2;
  if(rdy_head[USER_Q] == NIL_PROC)
  {
    rdy_head[USER_Q] = aux_proc;
    rdy_tail[USER_Q] = aux_proc;
    rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  }
  else
  {
    rdy_tail[USER_Q]->p_nextready = aux_proc;
    aux_proc->p_nextready = NIL_PROC;
    rdy_tail[USER_Q] = aux_proc;
  }
  return 0;
  */
  ...
  /*   ***********PIERWSZY SPOSOB************
  struct proc* to_modify;
  struct proc* to_move;
  struct proc* bef_modify;
  struct proc* bef_move;
  struct proc* aft_modify;
  struct proc* aft_move;


  struct proc* potential;
  struct proc* aux_potential;
  struct proc* x;
  struct proc* y;

  to_modify = rdy_head[USER_Q];
  bef_modify = NIL_PROC;

  while(to_modify != NIL_PROC && to_modify->p_pid != m_ptr->m1_i1)
  {
    bef_modify = to_modify;
    to_modify = to_modify->p_nextready;
  }

  if(to_modify == NIL_PROC)
    return(ESRCH);

  if(to_modify->NR_KOLEJKI == m_ptr->m1_i2)
    return 0;

  aft_modify = to_modify->p_nextready;

  if(bef_modify != NIL_PROC)
  {
    bef_modify->p_nextready = aft_modify;
    if(to_modify == rdy_tail[USER_Q])
      rdy_tail[USER_Q] = bef_modify;
  }
  else
  {
    rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  ...
  to_modify->p_nextready = NIL_PROC;

  while(1)
  {
    to_move = aft_modify;
    while(to_move != NIL_PROC && to_move->NR_KOLEJKI != to_modify->NR_KOLEJKI)
    {
      bef_move = to_move;
      to_move = to_move->p_nextready;
    }

    if(to_move == NIL_PROC || aft_modify == to_move)
      break;

    aft_move = to_move->p_nextready;

    if(bef_modify == NIL_PROC)
      rdy_head[USER_Q] = to_move;
    else
      bef_modify->p_nextready = to_move;

    to_move->p_nextready = aft_modify;
    bef_move->p_nextready = aft_move;

    if(aft_move == NIL_PROC)
      rdy_tail[USER_Q] = bef_move;

    bef_modify = bef_move;
    aft_modify = aft_move;
  }
  
  to_modify->NR_KOLEJKI = m_ptr->m1_i2;
   
  if(rdy_head[USER_Q] == NIL_PROC)
  {
    rdy_head[USER_Q] = to_modify;
    rdy_tail[USER_Q] = to_modify;
    rdy_tail[USER_Q]->p_nextready = NIL_PROC;
    return 0;
  }
 
   
  potential = rdy_head[USER_Q];
  aux_potential = NIL_PROC;
   
  while(potential != NIL_PROC && potential->NR_KOLEJKI <= to_modify->NR_KOLEJKI)
  {
    aux_potential = potential;
    potential = potential->p_nextready;
  }

  if(potential == NIL_PROC)
  {
    rdy_tail[USER_Q]->p_nextready = to_modify;
    to_modify->p_nextready = NIL_PROC;
    rdy_tail[USER_Q] = to_modify;
    return 0;
  }
  ...
  x = potential;
  y = potential->p_nextready;
  while(y != NIL_PROC && x->NR_KOLEJKI < y->NR_KOLEJKI)
  {
    x = x->p_nextready;
    y = y->p_nextready;
  }
  ...
  aux_potential->p_nextready = to_modify;
  to_modify->p_nextready = potential;
  */ 

  /*  **************LEPSZY AKTUALNY SPOSOB**************** */
  struct proc* to_modify;
  struct proc* bef_modify;
  struct proc* aft_modify;
  struct proc* aux_proc;
  struct proc* move_start;
  struct proc* move_stop;
  struct proc* bef_move_start;
  struct proc* bef_move_stop;
  ...
  aux_proc = rdy_head[USER_Q];
  while(aux_proc != to_modify)
  {
    if(aux_proc->NR_KOLEJKI > to_modify->NR_KOLEJKI)
    {
      undone = 0;
      break;
    }
  ...
  if(undone == 1)
  {
    bef_move_start = NIL_PROC;
    move_start = aux_proc->p_nextready;
    
    while(move_start != NIL_PROC && move_start->NR_KOLEJKI <= to_modify->NR_KOLEJKI)
    {
      if(move_start->NR_KOLEJKI == to_modify->NR_KOLEJKI)
      {
        next_exis = 1;
        break;
      }
      bef_move_start = move_start;
      move_start = move_start->p_nextready;
    }

    if(next_exis == 1)
    {
      bef_move_stop = move_start;
      move_stop = move_start->p_nextready;

      while(move_stop != NIL_PROC && bef_move_stop->NR_KOLEJKI < move_stop->NR_KOLEJKI)
      {
        bef_move_stop = move_stop;
        move_stop = move_stop->p_nextready;
      }
    }
  }
  ...
  {
    bef_modify->p_nextready = aft_modify; 
    if(aft_modify == NIL_PROC)
      rdy_tail[USER_Q] = bef_modify;
  }
  to_modify->p_nextready = NIL_PROC;

  
  if(undone == 1 && next_exis == 1 && bef_move_start != NIL_PROC) 
  {

    if(bef_modify == NIL_PROC)
    {
      rdy_head[USER_Q] = move_start;
      bef_move_stop->p_nextready = aft_modify;
      bef_move_start->p_nextready = move_stop;
      if(bef_move_stop == rdy_tail[USER_Q])
        rdy_tail[USER_Q] = bef_move_start;

    }
    else
    {
      bef_modify->p_nextready = move_start;
      bef_move_stop->p_nextready = aft_modify;
      bef_move_start->p_nextready = move_stop;
      if(bef_move_stop == rdy_tail[USER_Q])
        rdy_tail[USER_Q] = bef_move_start;
...
PRIVATE int do_getsizes(m_ptr)
message *m_ptr;
{
  return licznik_elementow;
}

PRIVATE int do_getheadnr(m_ptr)
message *m_ptr;
{
  if(rdy_head[USER_Q] == NIL_PROC)
    return -1;
  else
    return rdy_head[USER_Q]->NR_KOLEJKI;
}

PRIVATE int do_getheadpid(m_ptr)
message *m_ptr;
{
  if(rdy_head[USER_Q] == NIL_PROC)
    return -1;
  else
    return rdy_head[USER_Q]->p_pid;
}
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETSIZEF:	r = do_getsizef(&m);	break;
	    case SYS_GETSIZES:  r = do_getsizes(&m);	break;
	    case SYS_GETHEADNR: r = do_getheadnr(&m);	break;
	    case SYS_GETHEADPID: r = do_getheadpid(&m); break;
	    default:		r = E_BAD_FCN;
	}
