 ...
 */
  ...
  */
  ...
  served = rdy_head[USER_Q];
  rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;

  potential = rdy_head[USER_Q];
  aux_potential = NIL_PROC;
  while(potential != NIL_PROC && potential->NR_KOLEJKI <= served->NR_KOLEJKI)
  {
  ...
  {
    rdy_tail[USER_Q]->p_nextready = served;
    served->p_nextready = NIL_PROC;
    rdy_tail[USER_Q] = served;
    pick_proc();
    return;
  }
  ...
  aux_potential->p_nextready = served;
  served->p_nextready = potential;
